...
int n;
vector <ii> queen;
bool board[N][N], row[N], col[N], cross_m[N], cross_n[N];
int ans = +oo;
int cnt = 0;

bool MQueen(int i, int j){
    return (!col[j] && !row[i] &&
            !cross_m[j - i + 10] && !cross_n[(9 - j) - i + 10]);
}
...
void Calc(){
    ...
    sort(all(q));
    auto match = [&](vector <ii> queen, vector <ii> q) -> int{
        int ret = 0;
        for (int i = 0; i < n; ++i){
            int Qi = queen[i].first;
            int Qj = queen[i].second;
            int x = q[i].first;
            int y = q[i].second;
            int z = min(abs(Qi - x), abs(Qj - y));
            int k = max(abs(Qi - x), abs(Qj - y)) - z;
            ret += (z + k);
        }
        return ret;
    };
    ans = min(ans, match(queen, q));
...
void Get_Path(int j){
    ...
    for (int i = 1; i <= n; ++i){
        if (MQueen(i, j)) {
            ...
            row[i] = true;
            cross_m[j - i + 10] = true;
            cross_n[(9 - j) - i + 10] = true;
            board[i][j] = true;
            if (j == n) Calc();
                else Get_Path(j + 1);
            col[j] = false;
            row[i] = false;
            cross_m[j - i + 10] = false;
            cross_n[(9 - j) - i + 10] = false;
            board[i][j] = false;
        }
    };
}
