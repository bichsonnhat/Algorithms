...
void Solve(){
    ...
    for (int i = 0; i < q; ++i){
        ...
        while (true){
            ++indx;
            if (indx >= m) break;
            int w = adj[indx].first;
            if (w <= ask[i].first){
                int u = adj[indx].second.first;
                int v = adj[indx].second.second;
                if (!dsu.diff(u, v)) continue;
                dsu.join(u, v);
            } else {
                --indx;
                break;
        ...
        int res = 0;
        ans[ask[i].second.second] = cnt[dsu.find_set(ask[i].second.first)];
    }
    for (int i = 1; i <= q; ++i){
        cout << ans[i] << '\n';
    }
