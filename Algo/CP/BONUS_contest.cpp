...
void Subtask_DP(){
    ...
    for (int i = 1; i <= n; ++i){
        dp[i][0] = a[m][i];
        for (int j = 1; j <= n; ++j){
            dp[i][j] = compare(dp[i][j], dp[i][j - 1]);
            if (i - j > 0){
                dp[i][j] = compare(dp[i][j], dp[i][i - j] + a[m][i - j + 1]);
            }
            if (i + j <= n){
                dp[i][j] = compare(dp[i][j], dp[i][i + j] + a[m][i + j]);
            }
            ans = max(ans, abs(dp[i][j]));
        }
    }
    for (int i = 1; i <= n; ++i){
        for (int j = 0; j <= n; ++j){
