...
void Solve(){
    ...
    cin >> n;
    vector <int> a(n);
    long long ans = 0, sum_digs = 0, sum = 0;
    auto f = [&](int n) -> int{
        int cnt = 0;
        while (n){
            cnt ++;
    ...
    for (auto &x : a){
        cin >> x;
        sum = (sum + x) % MOD;
        sum_digs = (sum_digs % MOD + binPow(10, f(x), MOD) % MOD) % MOD;
    }
    for (auto i : a){
        long long curr_sum = sum_digs - binPow(10, f(i), MOD);
        curr_sum = (curr_sum + MOD) % MOD;
        long long s = (sum - i + MOD) % MOD;
        ans = ((ans % MOD) + (s % MOD) + (i * (curr_sum) % MOD)) % MOD;
//        add(ans);
    }
    cout << ans << '\n';
}
