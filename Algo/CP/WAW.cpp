...
void DFS(int u){
    ...
    for (auto v : adj[u]){
        ...
        int need = (int)(adj[u].size());
        if (need >= 1){
//            ans[u] += node;
            ans[u] = Merge(ans[u], node);
            if (status[u] != need){
                node.clear();
            } else {
                ans[u].push_back(val[u]);
                node = ans[u];
            }
        }
    }
    int deep = (int)(adj[u].size());
    if (!deep){
        node.push_back(val[u]);
//        ans[u] += node;
        ans[u] = Merge(ans[u], node);
    }
}
void Solve(){
    ...
    for (int i = 1; i <= n; ++i){
        ...
        int maxx = 0;
        for (auto x : ans[i]) ++cnt[x], maxx = max(maxx, cnt[x]);
//        unique(all(ans[i]));
        ll res = 0;
        for (auto x : cnt){
            if (x.second == maxx){
