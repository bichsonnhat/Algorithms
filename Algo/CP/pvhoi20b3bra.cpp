...
void Subtask_12(){
    while (q --> 0){
        int p; cin >> p;
        Swap(s[p - 1]);
        for (int i = 0; i < p; ++i){ /// We check Range [0..p - 1]
            Swap(s[i]); /// Change '(' <-> ')'
            if (i + 1 == p) { /// Maximum pos ! Not need to check with O(n)
                cout << i + 1 << ' ';
                break;
            }
            /// Optimize with case ")(", Not need to check with O(n)
            if (i) {
                char prev = s[i - 1]; /// Prev char
                if (prev == ')' && s[i] == '(') {
                    Swap(s[i]);
                    continue;
                }
