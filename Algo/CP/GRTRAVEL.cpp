...
void Subtask_1(){
    ...
    for (int i = 1; i <= n; ++i){
        for (int j = i + 1; j <= n; ++j){
            for (int k = j + 1; k <= n; ++k){
                ans += (board[i][j] && board[j][k] && board[k][i]);
                ans += (board[j][i] && board[i][k] && board[k][j]);
            }
        }
    }
    cout << ans << '\n';
...
//    }
    ...
    for (int i = 1; i <= n; ++i){
        for (auto u : adj[i]){
            for (auto v : adj[u]){
                ans += (g[{i, u}] && g[{u, v}] && g[{v, i}]);
                ans += (g[{u, i}] && g[{v, u}] && g[{i, v}]);
            }
        }
    }
    cout << ans << '\n';
